...
//   ~/.minicc/brain/graph.snapshot.json  完整快照(含 embedding)，仅在压缩时原子写一次
//   ~/.minicc/brain/graph.log.jsonl      增量日志，每次改动 append 一行(O(1)，不重写大文件)
//   ~/.minicc/brain/graph.json           旧格式(兼容读取/首启迁移的来源，保留作历史备份)
//
...
// 目录可用 MINICC_BRAIN_DIR 覆盖(自测用临时目录，不碰真实数据)
export const BRAIN_DIR = process.env.MINICC_BRAIN_DIR || join(homedir(), ".minicc", "brain");
