...
export function App() {
  ...
  const askAutoSecRef = useRef(askAutoSec);
  askAutoSecRef.current = askAutoSec;
  const stopRulesRef = useRef(stopRules);
  stopRulesRef.current = stopRules;
  ...
  async function babyRefresh() {
    try {
      const [di, cu] = await Promise.all([w.minicc.babyDiary(), w.minicc.babyCurious()]);
  ...
  async function toggleBabyAlive() {
    if (babyAlive) { setBabyAlive(false); try { await w.minicc.babyAliveStop(); } catch {} }
    ...
    const off = window.minicc.onEvent((ch, payload: any) => {
      ...
      switch (ch) {
        ...
        case "evt:session-loaded": {
          ...
          forceBottomRef.current = !jumping; // 切换会话：多帧兜底吸底，一次点击就到最新
  ...
  // 和上面的「切会话吸底」互斥(session-loaded 里已按 jumpRef 关掉 forceBottom)。
  useEffect(() => {
    ...
    stickingRef.current = true; // 跳转期间 onScroll 别把贴底态改来改去
    ...
    const step = () => {
      ...
      else stickingRef.current = false; // 兜底：找不到锚点(历史被删/压缩过)也别一直卡着
    };
  ...
  function startGoal(text: string) {
    ...
    setGoalEdit(null);
    // 这段话可以在设置里改；没有 {目标} 占位符就把目标接在开头
    const tpl = goalPromptOf();
    sendText(tpl.includes("{目标}") ? tpl.replaceAll("{目标}", t) : `【总目标】${t}\n\n${tpl}`);
  }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(async () => {
      try {
        const r = await window.minicc.searchSessions(q);
        if (dead) return;
        setSearchRes(r);
        setSearchSel(0);
      } catch {
  ...
  const gotoHit = (hit: import("./env").SearchHit) => {
    ...
    if (hit.sid !== currentIdRef.current) {
      window.minicc.switchSession(hit.sid);
      return;
    }
    ...
    <div className={"shell" + (showBrowser && !browserDetached && browserMode === "full" ? " browser-full" : "")}>
      ...
      <div className="sidebar" style={{ width: sidebarW }}>
        <div className="sidebar-top">
          <button className="icon-btn" title="搜索所有对话内容（⌘/Ctrl+F）" onClick={openSearch}>
            <SearchIcon />
          </button>
          ...
          {goal && (
            <div className={"goal-bar" + (goal.active ? " on" : "") + (goal.done ? " done" : "")}>
              ...
              {goal.active ? (
                <>
                  ...
                  <button
                    onClick={() => {
                      ...
                      window.minicc.goalSet?.(currentId, g);
                      setGoal(g);
                ...
                <>
                  {goal.done && <span className="goal-state">已完成</span>}
                  <button
                    className="on"
                    onClick={() => {
              ...
              {!goal.done && (
                <button
                  title="标记这个目标已达成（目标本身会一直留着，要清掉请到「改」里删除）"
                  onClick={() => {
            ...
            >
              ...
              {suggestWait > 0 ? (
                <button
                  ...
                  title="智能继续：这条它判成想问你一句，但没碰红线，所以倒数完还是会自动发。点这里改成等你。"
                  onClick={(e) => { e.stopPropagation(); setSuggestWait(0); }}
                >
                  {suggestWait}s 后自动发 · 点这等我
                </button>
              ) : (
        ...
        <>
          ...
          <div className="search-modal">
            <div className="search-bar">
              ...
              <button className="search-x" title="关闭（Esc）" onClick={() => setSearchOpen(false)}>
                ×
              </button>
            </div>
                  ...
                  >
                    <div className="search-row-top">
                      ...
                      <span className={"search-role " + h.role}>
                        {h.role === "user" ? "我" : h.role === "assistant" ? "AI" : "标题"}
                      </span>
                    ...
                    <div className="search-snip">
                      ...
                      <mark>{h.match}</mark>
                      {h.post}
                      {h.more > 0 && <span className="search-more">本条另有 {h.more} 处</span>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="goal-modal-b">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const g = t ? { text: t, active: false, done: false } : null;
                  window.minicc.goalSet?.(goalEdit.sid, g);
...
// 设置总目标时发出去的那段工作方式约定，可改可还原
function GoalPromptSettings() {
  const [text, setText] = useState(() => goalPromptOf());
  const [saved, setSaved] = useState(false);
  return (
    <>
      <div className="app-set-row" style={{ cursor: "default" }}>
        <div className="app-set-label">设置总目标后发给它的话</div>
        <button
          type="button"
          className="app-set-mini"
          onClick={() => {
            setText(DEFAULT_GOAL_PROMPT);
            localStorage.removeItem("minicc-goal-prompt");
            setSaved(true);
          }}
        >
          恢复默认
      ...
      <textarea
        ...
        onBlur={() => { localStorage.setItem("minicc-goal-prompt", text); setSaved(true); }}
      />
      <div style={{ fontSize: 11, opacity: .55, margin: "4px 0 14px", lineHeight: 1.5 }}>
        {saved ? "已保存。" : "改完点一下别处即保存。"}
        其中 <code>{"{目标}"}</code> 会被替换成你写的那个目标。这段话决定了它有多自主——
        想让它更放手就把第 4 条的红线收窄，想让它多汇报就加一条。
      </div>
    </>
  );
}
          ...
          {tab === "general" && (
            <>
              ...
              <div style={{ fontSize: 11, opacity: .55, marginBottom: 14, lineHeight: 1.5 }}>
                一行一条，优先级最高：命中就一定停下来问你，绝不自动继续、也不会替你选。
                已经预置了常见的（删除/上线/写生产/重启线上/花钱/对外发送/改权限/强推回滚/rm -rf 这些），按你的实际情况增删即可。
              </div>
              <GoalPromptSettings />
